...
import static org.friendsoftonga.akosipela.Start.MULTITYPE_TILES;
import static org.friendsoftonga.akosipela.Start.SILENT_PRELIMINARY_TILES;
import static org.friendsoftonga.akosipela.Start.colorList;
import static org.friendsoftonga.akosipela.Start.differentiatesTileTypes;
import static org.friendsoftonga.akosipela.Start.gameList;
import static org.friendsoftonga.akosipela.Start.stageCorrespondenceRatio;
...
import static org.friendsoftonga.akosipela.Start.placeholderCharacter;
import static org.friendsoftonga.akosipela.Start.tileHashMap;
import static org.friendsoftonga.akosipela.Start.tileList;
import static org.friendsoftonga.akosipela.Start.tileStagesLists;
import static org.friendsoftonga.akosipela.Start.wordList;
import static org.friendsoftonga.akosipela.Start.wordStagesLists;
import static org.friendsoftonga.akosipela.Testing.tempSoundPoolSwitch;
...
import static org.friendsoftonga.akosipela.Start.after12checkedTrackers;
...
public abstract class GameActivity extends AppCompatActivity {
    ...
    protected MediaPlayer preloadedPlayer;

    // Add this variable near your other MediaPlayer variables
    public boolean shouldPlayPromptAsap = false;

    public void playInitialPrompt(String audioName) {
        int resID = getResources().getIdentifier(audioName, "raw", getPackageName());
        if (resID != 0) {
            MediaPlayer mp = MediaPlayer.create(this, resID);
            if (mp != null) {
                mediaPlayerIsPlaying = true; // This locks the screen
                mp.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                    @Override
                    public void onCompletion(MediaPlayer mediaPlayer) {
                        mediaPlayerIsPlaying = false; // <--- ADD THIS: This unlocks the screen
                        mediaPlayer.release();
                    }
                });
                mp.start();
    ...
    public void preloadGeneralAudio(String audioName, final boolean playImmediately) {
        int resID = getResources().getIdentifier(audioName, "raw", getPackageName());
        if (resID != 0) {
            try {
                preloadedPlayer.reset();
                android.content.res.AssetFileDescriptor afd = getResources().openRawResourceFd(resID);
                preloadedPlayer.setDataSource(afd.getFileDescriptor(), afd.getStartOffset(), afd.getLength());
                afd.close();

                preloadedPlayer.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
                    @Override
                    public void onPrepared(MediaPlayer mp) {
                        // This is the "Alphabet Delay" fix.
                        // It only runs once Android has actually found and loaded "tuli".
                        if (playImmediately) {
                            mp.start();
                        }
                    }
                ...
                // This is non-blocking, so the game doesn't freeze while searching
                preloadedPlayer.prepareAsync();
    ...
    protected void mpCompletion(MediaPlayer mp, boolean isFinal) {
        mediaPlayerIsPlaying = false; // Reset every time a sound finishes
        if (isFinal) {
            updatePointsAndTrackers(0);
            repeatLocked = false;
            playCorrectFinalSound();
    ...
    protected void playWordAudio(Start.Word wordObject, @Nullable Runnable onComplete) {
        ...
        try {
            MediaPlayer mp = MediaPlayer.create(this, resID);
            if (mp == null) {
                // If create fails, reset state so game doesn't freeze
                mediaPlayerIsPlaying = false;
                setAllGameButtonsClickable();
                if (onComplete != null) onComplete.run();
                return;
            ...
            mp.setOnCompletionListener(mediaPlayer -> {
                ...
                setAllGameButtonsClickable();
                setOptionsRowClickable();
                mediaPlayer.release(); // Crucial: free up memory immediately
                if (onComplete != null) onComplete.run();
            });

            mp.start();
        } catch (Exception e) {
            Log.e("GameActivity", "Audio failed: " + e.getMessage());
            mediaPlayerIsPlaying = false;
            setAllGameButtonsClickable();
            if (onComplete != null) onComplete.run();
        }
