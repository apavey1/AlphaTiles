...
public class Start extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        android.util.Log.d("!!!TARGET!!!", "Starting Load...");

        // --- TIMER LOAD LOGIC ---
        String savedData = null;
        try {
            java.io.InputStream is = getAssets().open("raw_data.txt");
            int size = is.available();
            byte[] buffer = new byte[size];
            is.read(buffer);
            is.close();
            savedData = new String(buffer, "UTF-8");
        } catch (java.io.IOException ex) {
            android.util.Log.e("!!!TARGET!!!", "FILE NOT FOUND");
        }

        if (savedData != null && !savedData.isEmpty()) {
            wordList = new Start.WordList();
            String[] words = savedData.split("@@");
            for (String entry : words) {
                String[] p = entry.split("##");
                if (p.length >= 6) {
                    wordList.add(new Start.Word(p[0], p[1], Integer.parseInt(p[2]), p[3], p[4], p[5]));
                }
            }
            android.util.Log.d("!!!TARGET!!!", "Words loaded at: " + (System.currentTimeMillis() - startTime) + "ms");

            buildTileStagesLists();
            android.util.Log.d("!!!TARGET!!!", "Tiles built at: " + (System.currentTimeMillis() - startTime) + "ms");

            buildWordStagesLists();
            android.util.Log.d("!!!TARGET!!!", "Word Stages built at: " + (System.currentTimeMillis() - startTime) + "ms");

            buildWordStagesLists();
            android.util.Log.d("!!!TARGET!!!", "Word Stages built at: " + (System.currentTimeMillis() - startTime) + "ms");

            buildGameList(); // <--- Add a timer for this
            android.util.Log.d("!!!TARGET!!!", "Game List built at: " + (System.currentTimeMillis() - startTime) + "ms");
